...
export default function AdminVacancyInquiriesPage() {
  const [inquiries, setInquiries] = useState<VacancyInquiry[]>([]);
  const [selected, setSelected] = useState<VacancyInquiry | null>(null);
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const { sortedItems, sortKey, direction, requestSort } = useSortable(inquiries);

  const fetchInquiries = () => {
    api.get("/admin/vacancy-inquiries?size=100").then((res) => {
      const list: VacancyInquiry[] = res.data.content || [];
      list.sort((a, b) => {
  ...
  const handleMarkRead = async (id: number) => {
    try {
      await api.put(`/admin/vacancy-inquiries/${id}/read`);
      toast({ title: "已標記為已讀" });
      fetchInquiries();
    } catch {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/admin/vacancy-inquiries/${deleteId}`);
      toast({ title: "已刪除" });
      setDeleteId(null);
      fetchInquiries();
    } catch {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">職位查詢</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <SortHeader label="地區偏好" sortKey="districtPreference" currentKey={sortKey} direction={direction} onSort={requestSort} />
          ...
          <tbody className="divide-y divide-slate-100">
            {sortedItems.map((q) => (
              <tr key={q.id} className={`hover:bg-slate-50/60 transition-colors group ${!q.isRead ? "bg-blue-50/40" : ""}`}>
                ...
                <td className="px-5 py-4">
                  <span className="font-medium text-slate-900">{q.firstName} {q.lastName}</span>
                </td>
                <td className="px-5 py-4 text-slate-500">{q.phone || "—"}</td>
                <td className="px-5 py-4 text-slate-500">{q.serviceType || "—"}</td>
                <td className="px-5 py-4 text-slate-500">{q.districtPreference || "—"}</td>
              ...
              <div className="px-6 pt-6 pb-4 border-b">
                <DialogHeader className="mb-1">
                  <div className="flex items-center gap-3">
                    <DialogTitle className="text-lg">職位查詢詳情 #{selected.id}</DialogTitle>
                    {selected.isRead ? (
              ...
              <div className="px-6 py-5 space-y-6">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                    <InfoRow icon={User} label="名字">
                      {selected.firstName}
                    </InfoRow>
                    <InfoRow icon={User} label="姓氏">
                      {selected.lastName}
                    </InfoRow>
                    ...
                    <InfoRow icon={MapPin} label="地區偏好">
      ...
      <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-slate-600">
            確定要刪除查詢「<span className="font-semibold text-slate-900">{pendingInquiry?.firstName} {pendingInquiry?.lastName}</span>」嗎？此操作無法復原。
          </p>
