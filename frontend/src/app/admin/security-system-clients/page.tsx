...
export default function AdminSecuritySystemClientsPage() {
  const [clients, setClients] = useState<SecuritySystemClient[]>([]);
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const { sortedItems, sortKey, direction, requestSort } = useSortable(clients);

  const fetchClients = () => {
    api.get("/admin/security-system-clients").then((res) => {
      setClients(res.data);
    });
  };
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/admin/security-system-clients/${deleteId}`);
      toast({ title: "已刪除" });
      setDeleteId(null);
      fetchClients();
    } catch {
              ...
              <tr key={c.id} className="hover:bg-slate-50/60 transition-colors group">
                ...
                <td className="px-5 py-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/admin/security-system-clients/${c.id}/edit`}>
