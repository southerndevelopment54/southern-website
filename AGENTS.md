...
- **Infrastructure** (`docker-compose.yml`, `nginx/`): Dockerised PostgreSQL, MinIO, and Nginx reverse proxy
...
- **Secrets management**: JWT signing key is loaded from environment variables (`.env`) at startup
...
```bash
# 1. Start PostgreSQL, MinIO, Nginx
...
- **Signing key**: Loaded from environment variable `JWT_SECRET` (min 64 chars)
...
- **JWT secret**: Must be at least 64 characters; generated by `init-secrets.sh`
...
3. Ensure all secrets in `.env` are strong and unique
...
- **Backend fails to start**: Check that `.env` has all required values and `docker compose` loaded them correctly
